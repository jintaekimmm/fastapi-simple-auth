...
from crud.crud_user import UserDAL
...
async def api_token_refresh(*,
    ...
    saved_token: JWTToken = await token_dal.get(refresh_token_key=Hasher.hmac_sha256(refresh_token))
    if not saved_token:
        return ErrorJSONResponse(message='인증 토큰이 존재하지 않습니다',
                                 success=False,
                                 error_code=1404,
    ...
    login_user: User = await user_dal.get_by_user_id(user_id=saved_token.user_id)
    if not login_user:
    ...
    new_token: token.TokenSchema = await create_new_jwt_token(sub=str(saved_token.user_id))
    new_token.refresh_token = aes.decrypt(saved_token.refresh_token)

    update_token = token.TokenUpdateSchema(user_id=saved_token.user_id,
    ...
    try:
        ...
        return ErrorJSONResponse(message='토큰 갱신을 실패하였습니다',
                                 success=False,
                                 error_code=1500,
