...
from crud.crud_token import TokenDAL
from crud.crud_user import UserDAL, UserLoginHistoryDAL
...
from schemas.responses import ErrorResponse, DefaultResponse
from schemas.token import TokenUpdateSchema, TokenAccessOnlySchema
...
@router.post('/register',
             response_model=register.RegisterResponseSchema,
             responses={
                 404: {
                     'model': ErrorResponse
                 },
...
async def register_user(*,
                        register_request: register.RegisterRequestSchema,
    ...
    if await user_dal.exists_email(email_key=email_key):
        return ErrorJSONResponse(message='사용할 수 없는 이메일 주소입니다',
                                 success=False,
    ...
    mobile_key: str | None = Hasher.hmac_sha256(register_request.mobile) if register_request.mobile else None
    # 핸드폰 번호를 입력하였다면, 이미 등록된 핸드폰 번호인지 확인한다
    if register_request.mobile:
        if await user_dal.exists_mobile(mobile_key=mobile_key):
            return ErrorJSONResponse(message='사용할 수 없는 핸드폰 번호입니다',
                                     success=False,
                                     status_code=status.HTTP_409_CONFLICT,
                                     error_code=1409)
        else:
    ...
    new_register = register.RegisterInsertSchema(name=register_request.name,
    ...
    try:
        ...
        return ErrorJSONResponse(message='회원가입을 처리하는 도중에 문제가 발생하였습니다',
                                 success=False,
    ...
    response = register.RegisterResponseSchema(id=new_user_id,
                                               success=True,
                                               message='회원가입을 환영합니다')
...
@router.post('/api/login',
             response_model=token.TokenSchema,
             responses={
                 400: {
                     'model': ErrorResponse
                 },
...
async def api_login(*,
                    login_request: login.LoginSchema,
                    request: Request,
                    session: AsyncSession = Depends(get_session)):
    """
    Login API
    ...
    aes = AESCipher()
    user_dal = UserDAL(session=session)
    user_login_dal = UserLoginHistoryDAL(session=session)
    token_dal = TokenDAL(session=session)

    try:
        login_user: User = await user_dal.get_by_email(email_key=Hasher.hmac_sha256(login_request.email))

    except TypeError as e:
        logger.info(f'사용자를 조회하는 도중에 문제가 발생하였습니다. { {"email": masking_str(login_request.email)} }')
        logger.exception(e)
        return ErrorJSONResponse(message='사용자를 조회하는 도중에 문제가 발생하였습니다',
                                 success=False,
                                 status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                                 error_code=1500)
    ...
    if not login_user.is_active:
        logger.info(f'사용할 수 없는 아이디 입니다. { {"email": masking_str(login_request.email)} }')
        return ErrorJSONResponse(message='사용할 수 없는 아이디 입니다',
                                 success=False,
                                 status_code=status.HTTP_400_BAD_REQUEST,
                                 error_code=1400)

    if not await authenticate(plain_password=login_request.password,
                              user_password=login_user.password):
        logger.info(f'사용자 인증에 실패하였습니다. { {"email": masking_str(login_request.email)} }')
        return ErrorJSONResponse(message='사용자 인증에 실패하였습니다',
                                 success=False,
                                 status_code=status.HTTP_403_FORBIDDEN,
                                 error_code=1403)

    # JWT Token쌍을 생성한다
    new_token: token.TokenSchema = await create_new_jwt_token(sub=str(login_user.id))

    # 생성한 RefreshToken을 DB에 저장하기 위한 스키마 생성
    new_refresh_token = token.TokenInsertSchema(user_id=login_user.id,
    ...
    try:
        ...
        # Login 이력 저장
        await user_login_dal.insert_login_history(login_history=login.LoginHistorySchema(user_id=login_user.id,
        ...
        logger.exception(e)
        await session.rollback()
        return ErrorJSONResponse(message='로그인을 하는 도중에 문제가 발생하였습니다',
                                 success=False,
                                 status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                                 error_code=1500)
    finally:
    ...
    logger.info(f'사용자가 로그인하였습니다. { {"user_id": login_user.id, "email": masking_str(login_request.email), "name": masking_str(login_user.name)} }')

    response = token.TokenSchema(**new_token.dict())
...
async def api_logout(*,
                     user_token: token.AuthTokenSchema = Depends(AuthorizeToken()),
                     session: AsyncSession = Depends(get_session)):
    """
    Logout API
    ...
    try:
        if await token_dal.exists(user_id=int(user_token.sub), access_token=user_token.access_token):
            await token_dal.delete(user_id=int(user_token.sub), access_token=user_token.access_token)
        ...
        logger.exception(e)
        await session.rollback()
        return ErrorJSONResponse(message='토큰 정보를 찾을 수 없습니다',
                                 success=False,
                                 status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                                 error_code=1500)
    finally:
    ...
    return DefaultJSONResponse(message='로그아웃 하였습니다',
                               success=True)
...
@router.post('/web/login',
             response_model=token.TokenAccessOnlySchema,
             responses={
                 400: {
                     'model': ErrorResponse
                 },
...
async def web_login(*,
    ...
    token_response = token.TokenAccessOnlySchema(**new_token.dict())
    # accessToken은 json으로 반환한다
    response = JSONResponse(content=token_response.dict())
    # refreshToken은 Cookie로 반환한다
    response.set_cookie(key='refresh_token', value=f'{new_token.refresh_token}', httponly=True, secure=True, samesite='none')
...
async def web_logout(*,
    ...
    response = DefaultJSONResponse(message='로그아웃 하였습니다',
                                   success=True)
